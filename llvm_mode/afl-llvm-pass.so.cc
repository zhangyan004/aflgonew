    ...
    if (!TargetsFile.empty()) {
        ...
        is_aflgo_preprocessing = true;
             ...
             if (cf.is_open()) {
				...
				std::ifstream cff (BasicBlockInfoFile);
		        if (cff.is_open()) {
    ...
    for (auto &F : M) {
      ...
      for (auto &BB : F) {
		...
		if (number > 0){
			...
			#ifdef __x86_64__
			IntegerType *LargestType = Int64Ty;
			ConstantInt *MapCurLoc = ConstantInt::get(LargestType, MAP_SIZE + 8 + ((upre_number - 2) * MATRIX_SIZE + unumber - 1) * 8);
			#else
			IntegerType *LargestType = Int32Ty;
			ConstantInt *MapCurLoc = ConstantInt::get(LargestType, MAP_SIZE + 4 + ((upre_number - 2) * MATRIX_SIZE + unumber - 1) * 4);
			#endif
